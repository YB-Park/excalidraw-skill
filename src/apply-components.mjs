...
function text(id, value, x, y, width, size = 11) {
  ...
  t.containerId = null;
  t.lineHeight = 1.25;
  t.strokeColor = '#475569';
  return t;
}

function tag(node, value, color, fill) {
  const box = make('rectangle', safeId('tag', `${node.id}_${value}`), node.x + 12, node.y + 8, 52, 18);
  box.strokeColor = color;
  box.backgroundColor = fill;
  box.strokeWidth = 1;
  const label = text(safeId('tag_text', `${node.id}_${value}`), value, box.x + 8, box.y + 2, 44, 10);
...
function line(node, name, x1, y1, x2, y2, color) {
  ...
  l.points = [[0, 0], [x2 - x1, y2 - y1]];
  l.strokeColor = color;
  l.customData.excalidrawSkill.node = node.customData?.excalidrawSkill?.semanticId;
  return l;
}
...
function queue(node) {
  ...
  const label = text(safeId('queue_label', node.id), 'EVENT', node.x + 12, node.y + 4, 60, 10);
  label.strokeColor = '#7e22ce';
  const dots = [0, 1, 2].map((i) => {
    const dot = make('ellipse', safeId(`queue_dot_${i}`, node.id), node.x + node.width - 44 + i * 14, node.y + 8, 8, 8);
...
function risk(node) {
  ...
  const bang = text(safeId('risk_bang', node.id), '!', badge.x + 8, badge.y + 1, 12, 15);
  bang.strokeColor = '#b45309';
  return [...accent(node, '#d97706', 10), badge, bang];
}

function worker(node) {
  const badge = make('rectangle', safeId('worker_badge', node.id), node.x + node.width - 54, node.y + 8, 42, 18);
  badge.strokeColor = '#7c3aed';
  badge.backgroundColor = '#ede9fe';
  const label = text(safeId('worker_label', node.id), 'JOB', badge.x + 9, badge.y + 2, 30, 10);
  label.strokeColor = '#6d28d9';
  return [...accent(node, '#7c3aed'), badge, label];
}
...
function run() {
  ...
  const additions = [];
  const existing = new Set((scene.elements ?? []).map((e) => e.id));

  for (const element of scene.elements ?? []) {
    const meta = element.customData?.excalidrawSkill;
    if (meta?.role !== 'node') continue;
    for (const detail of details(element)) {
