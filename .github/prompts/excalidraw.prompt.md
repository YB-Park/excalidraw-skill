...
- When image vision is available, inspect the PNG reported by `review` using `guides/visual-review.md`; do not judge aesthetics from metrics alone.
- Inspect the image before reading suggested fixes. Fix blocker/major visual defects with at most two bounded refinement passes.
...
- Never use low-level `render` for PNG output. `render` writes Excalidraw JSON only and is developer-only.
