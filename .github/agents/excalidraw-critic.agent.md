...
For each candidate, independently call `excalidraw/diagram_review_image` and inspect the returned PNG. Evaluate each candidate before comparing them. Do not use strategy names as evidence.
...
- `humanDecisionRecommended: true` when confidence is low, top candidates are close, or the diagram is presentation-critical
